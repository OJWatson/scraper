...
of PDFs, which sadly are not formatted for search. An example of this can be seen
...
2015. These PDFs contain a variety of information, but for example let's try and
focus on the *"Reported confrmed cases:"*. 

Before we start we need to create a function to identify all the link urls. In doing 
this and then scraping from the pdfs we will requite a number of very useful R
packages:
...
# Simplifies R code for passing data munging forward
install.packages("magrittr",repos="http://cloud.r-project.org/")
...
URL_Lists_From_CSS_Selector <- function(url,css){
  ...
  ## that will not provide the complete url
...
the desired PDFs. We now want to download and extract the number that appears
after *"Reported confrmed cases:"*. We will want to carry this out suppressing
errors as it is quite common for HTTP errors to occur, or PDF to text conversions
that will throw errors but that will not affect what we are interested in. We will 
then want to identify where our text belongs and then grab the following words.

Before looking at this, it is worth mentioning that scraping is ususally quite ugly...  
...
## and what number of words that we are interested in after the string. 
...
col.names = c("Cases")
# How many characters after teh string should we search for the value we want
search.length = 200
...
PDFs and see what the alternatives
...
```{r Alternative String, include=TRUE, message = FALSE, warning = FALSE,results='hide'}
...
```{r formatting, include=TRUE, message = FALSE, warning = FALSE,results='hide'}
...
information we want, although it quite messy. It is also worth noting that this
is just what I have found possible from what I have discovered in R in the past 
few weeks, and for example have found sources for alternative tools.For example,
...
inspiration and I hope that these can give perahps better indications of what could
be achieved. 
